...
class CollectionPreview extends React.Component {
    ...
    constructor(props) {
        super(props);
        console.log('In COLLECTION PREVIEW GRID: this.props is', this.props)
        this.state = { items: [], title: this.props.images[0].title };
    ...
    componentDidMount() {
        let items = Array.apply(null, this.props.images).map((v, i) => {
            console.log('NEWGRID v.uri ', v.uri);
            console.log('NEWGRID i ', i);
            if (i <= 1) {
                return { id: i, src: v.uri }
            }
        });
    ...
    renderFooter() {
        return(
            <View style={{height: 75, backgroundColor: 'steelblue'}}>
                <Text style={[styles.bannerTop, styles.baseText]}>{this.props.collectionInfo.title}</Text>
                <Text style={[styles.bannerBottom, styles.baseText]}>{this.props.collectionInfo.author}</Text>
            </View>
        );
    }
    ...
    renderItem(item, itemSize) {
        return(
            <TouchableOpacity
                ...
                onPress = {() => {this.props.navigation.navigate('Collection', {images: this.props.images, collection: this.props.collectionInfo})
